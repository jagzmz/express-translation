...
'use strict';
const { find, filter, map, get, isFunction } = require('lodash');
    ...
    return function etMiddleware(req, res, next) {
        ...
        if (!req.get(self.triggerHeader)) {
            ...
            res.json = function (body) {
                ...
                const processedTranslation = get(
                    req,
                    'res.locals.processedTranslation'
                );
                if (processedTranslation && isFunction(processedTranslation)) {
                    processedTranslation(body, this);
                }
            };
            return next();
        }
