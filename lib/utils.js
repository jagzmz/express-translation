...
const makeSchema = function (schema, key) {
    ...
    if (typeof schema[key] === 'string') {
        try {
            ...
            schema[key] = appRootPath.require(
                `${schema[key]}.translation.json`
            );
        } catch (error) {
            throw new Error(
...
const makeConfig = function (config, configPath) {
    const oldPath = appRootPath.path;
    appRootPath.setPath(appRootPath.path + '/' + configPath);
    for (const path in config) {
        makeSchema(config[path], 'schema');
        makeSchema(config[path], 'response');
    }
    fs.writeFileSync(
        appRootPath.path + '/schemaPreview.json',
        JSON.stringify(config)
    );
    appRootPath.setPath(oldPath);
};
