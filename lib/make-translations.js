...
const replaceBodyWithOriginalKeys = function (
    ...
    if (!this.serialize) {
        if (this.methodSignature === 'GET') {
            ...
            dataBody[fieldName] =
                dataBody[translatedFieldName] || dataBody[fallbackField]; //||
            //dataBody[fieldName];
        } else {
            dataBody[fieldName] = dataBody[translatedFieldName]; // if method is has not translation field must always be there //|| dataBody[fieldName];
        }
    }
